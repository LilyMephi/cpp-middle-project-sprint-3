...
TEST(BookFormatterTest, BasicFormatting) {
    ...
    std::string result = fmt::format("{}", sci_fi_book);
    EXPECT_EQ(result, 
...
TEST(BookFormatterTest, EmptyFields) {
    ...
    std::string result = fmt::format("{}", empty_title);
    EXPECT_EQ(result, 
        "Book(title=\"\", author=\"Author\", year=2000, genre=Fiction, rating=3.5, read_count=1)");
}
...
TEST_F(BookDatabaseTesting, AuthorHistogram) {
    ...
    EXPECT_EQ(10, histogram.size());
    // auto it = histogram.find("George Orwell");
...
TEST_F(BookDatabaseTesting, AverageRating) {
    ...
    EXPECT_NEAR(4.49, avgRating, 0.01);  // (4.0+4.4+4.5+4.8+4.7+4.3+4.5+4.6+4.9+4.2)/10
}
...
TEST_F(BookDatabaseTesting, TopNBooks) {
    ...
    ASSERT_NE(db.end(), orwellBookIt);
   	EXPECT_TRUE(orwellBookIt->title == "1984" || orwellBookIt->title == "Animal Farm");
}
